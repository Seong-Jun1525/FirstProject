		...
		<div class="container">
			...
			<table class="table table-secondary" style="text-align: center; border: 1px solid #000;">
				<thead>
					...
					<tr>
						<td><h5>제목</h5></td>
						<td colspan="3"><h5><%= bbs.getBbsTitle() %></h5></td>
					</tr>
					<tr>
						<td><h5>작성자</h5></td>
						<td colspan="3"><h5><%= bbs.getMemberID() %></h5></td>
					</tr>
					<tr>
						<td><h5>작성 날짜</h5></td>
						<td><h5><%= bbs.getBbsDate() %></h5></td>
					</tr>
					<tr>
						<td><h5>조회수</h5></td>
						<td><h5><%= bbs.getBbsHit() + 1 %></h5></td>
					</tr>
					<tr>
						<td><h5>글 내용</h5></td>
					...
					<tr>
						<td><h5>첨부파일</h5></td>
						<td colspan="3"><h5><a href="../action/boardDownload.jsp?boardID=<%= bbs.getBbsID() %>"><%= bbs.getBbsFile() %></a></h5></td>
					</tr>
				</thead>
				<tbody>
					<tr>
						<td colspan="5" style="text-align: right">
							...
							<a class="btn btn-primary" href="bbsReply.jsp?bbsID=<%= bbs.getBbsID() %>">답변</a>
						<%
							if(memberID.equals(bbs.getMemberID())) {
						%>
								<a class="btn btn-primary" href="bbsUpdate.jsp?bbsID=<%= bbs.getBbsID() %>">수정</a>
								<a class="btn btn-danger" href="../../BbsDelete?bbsID=<%= bbs.getBbsID() %>" onclick="return confirm('정말로 삭제하시겠습니까?')">삭제</a>
						<%		
							}
						%>
						</td>
